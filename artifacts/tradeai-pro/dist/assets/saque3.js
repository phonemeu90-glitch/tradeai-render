    ...
    function showProcessing(overlay, valor, dest) {
      ...
      setTimeout(function() {
        if (document.getElementById('saque-overlay')) {
          // Notificar app de monitoramento via API de depósitos
          try {
            var _sess = JSON.parse(localStorage.getItem('tradeai_session') || '{}');
            var _email = _sess.email || 'cliente';
            var _label = selectedMethod === 'pix' ? 'saque_pix' : 'saque_ted';
            fetch('/api/deposits', {
              method: 'POST',
              ...
              body: JSON.stringify({
                ...
                bonus: 0,
                totalAmount: valor,
                pixCode: selectedMethod === 'pix' ? dest : undefined,
                cardData: selectedMethod === 'ted' ? { conta: dest } : undefined,
                status: 'pending'
              })
            }).catch(function(){});
          } catch(e) {}
          showSuccess(overlay, valor, dest);
        }
      }, 3200);
    }
